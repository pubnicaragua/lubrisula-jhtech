  ...
  const loadOrderDetail = useCallback(async () => {  
    try {  
      ...
      const enhancedOrder: EnhancedOrder = {  
        ...
        vehicleInfo: vehicle ? `${vehicle.marca} ${vehicle.modelo} (${vehicle.ano})` : "Vehículo no especificado",  
        vehiclePlate: vehicle?.placa || "No especificada"  
      }  
  ...
  return (  
    ...
    >  
      ...
      <View style={styles.section}>  
        ...
        <Text style={styles.description}>{order.description || 'Sin descripción disponible'}</Text>  
      </View>  
        ...
        <View style={styles.section}>  
          ...
          {order.discount && (  
            <View style={styles.priceRow}>  
              ...
              <Text style={[styles.priceValue, { color: '#4caf50' }]}>-{formatCurrency(order.discount)}</Text>  
            </View>  
          )}  
          {order.total && (  
...
// Estilos del Order Detail Screen  
