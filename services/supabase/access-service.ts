...
export const accessService = {  
  ...
  // ✅ CORREGIDO: Método principal con consistencia de 'role'  
  async GET_PERMISOS_USUARIO(userId: string, tallerId: string): Promise<UserPermissions | null> {  
    try {  
      // Consultar el rol real del usuario desde la base de datos  
  ...
  .eq('auth_id', userId)  
      ...
      if (error) {  
        ...
        return {  
          role: 'client', // ✅ CORREGIDO: Usar 'role' consistentemente  
          permisos: [],  
          taller_id: tallerId  
        }  
      }  
      ...
      return {  
        role: profile.role || 'client', // ✅ CORREGIDO: Usar 'role' consistentemente  
        permisos: [],  
        taller_id: profile.taller_id || tallerId  
      }  
    } catch (error) {  
